        ...
        <div class="table-responsive text-nowrap">
            <table class="table">
                ...
                @foreach ($modules as $module)
                    <tr data-bs-toggle="collapse" data-bs-target="#module_{{ $module->code }}" aria-expanded="false"
                        aria-controls="module_{{ $module->code }}">
                        <td><button class="btn btn-default btn-xs clickable"><img
                                    src="https://cdn-icons-png.flaticon.com/128/8567/8567254.png" width="20px"
                                    alt=""></button></td>
                        <td>{{ $module->name }}</td>
                        ...
                        <td>
                            <form method="get" action="{{ route('pages-modules') }}">
                                ...
                                <input type="hidden" name="module_status"
                                    value="{{ $module->is_active ? 'inactive' : 'active' }}">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox"
                                        id="activeSwitch_{{ $module->code }}" name="is_active"
                    ...
                    <tr>
                        <td colspan="6" class="hiddenRow">
                            <div class="accordian-body collapse" id="module_{{ $module->code }}">
                                <table class="table">
                                    <thead
                                        style="background: linear-gradient(to right, rgb(209, 191, 230), #D3CCED); color: white;">
                                        <tr>
                                            <th></th>
                                            <th>Name</th>
                                            <th>Description</th>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <form method="get" action="{{ route('pages-modules') }}">
                                                        ...
                                                        <input type="hidden" name="module_status"
                                                            value="{{ $submodule->is_active ? 'inactive' : 'active' }}">
                                                        <div class="form-check form-switch">
                                                            <input class="form-check-input" type="checkbox"
                                                                id="activeSwitch_{{ $submodule->code }}" name="is_active"
                                                                {{ $submodule->is_active ? 'checked' : '' }}
                                                                onchange="this.form.submit()">
                                                            <label class="form-check-label"
                                                                for="activeSwitch_{{ $submodule->code }}"></label>
                                                        </div>
                                                    </form>
                                                </td>
